...
const importanceColor = { high: '#dc2626', medium: '#d97706', low: '#9ca3af' }
...
export default function Home() {
  ...
  async function startFetch() {
    ...
    const msgs = [
      ...
      'Identifying which specs matter most for buying decisions…',
      'Building the recommended spec row…',
    ]
    let mi = 0
  ...
  function startOver() {
    ...
    setInputError(''); setFatalError(''); setSynthesizing(false)
  }
  ...
  function getDevJSON() {
    ...
    return JSON.stringify({
      ...
      specFields: matrixData.specRows.filter(r => r.includeInRecommended).map(r => ({
        key: (r.recommendedLabel || r.concept).toLowerCase().replace(/[^a-z0-9]+/g, '_'),
        label: r.recommendedLabel,
        value: r.recommendedValue,
        importance: r.buyingDecisionImportance,
      })),
    }, null, 2)
  }

  function copyJSON() {
    navigator.clipboard.writeText(getDevJSON()).then(() => { setCopied(true); setTimeout(() => setCopied(false), 1500) })
  }
        ...
        <div style={{ maxWidth: 1200, margin: '0 auto' }}>
          ...
          <div style={{ marginBottom: '2rem' }}>
            ...
            <p style={{ fontSize: 14, color: '#6b7280' }}>
              Paste product page URLs → extract verbatim specs → get a buying-decision spec list for your database
            </p>
          </div>
            ...
            {step === 1 && (
              <div>
                ...
                <p style={{ fontSize: 12, color: '#9ca3af', margin: '1rem 0', lineHeight: 1.6 }}>
                  Every spec shown will be extracted verbatim from the actual page — nothing is invented. Amazon URLs are often blocked even with scraping; try Best Buy, Target, Wayfair, AJ Madison, Lowe's first.
                </p>
                  ...
                  {statusList.map(([url, s]) => (
                    <div key={url} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '11px 16px', borderBottom: '1px solid #f3f4f6' }}>
                      ...
                      <span style={{ fontSize: 12, color: s.status === 'done' ? '#10b981' : s.status === 'error' ? '#ef4444' : '#9ca3af' }}>
                        ...
                        {s.status === 'loading' && 'Fetching & extracting…'}
              ...
              <div>
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '1.25rem' }}>
                  ...
                  <div style={{ display: 'flex', gap: 8 }}>
                    <button onClick={copyJSON} style={ghostBtn}>{copied ? 'Copied!' : 'Copy JSON for devs'}</button>
                    <button onClick={startOver} style={ghostBtn}>Start over</button>
                  </div>
                </div>
                ...
                <p style={{ fontSize: 12, color: '#9ca3af', marginTop: 12, lineHeight: 1.6 }}>
                  Every value above was extracted verbatim from the linked product page — nothing is invented. Blank cells mean that retailer's page didn't show that spec. The Recommended column is blank where a spec isn't useful for a buying decision (model numbers, SKUs, etc).
                </p>
              </div>
            )}
